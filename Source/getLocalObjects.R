...
getLocalObjects <- function( habitatMask,
  ...
  habitatCoords <- habitatCoords[order(habitatCoords[ ,2], habitatCoords[ ,1]), ]
  ...
  if(any(numLocalIndices %in% 0)){
     warnings("dmax value too small. All habitat grid cells should have at least one local object within a radius of dmax.")
   }
