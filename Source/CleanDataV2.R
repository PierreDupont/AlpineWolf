...
CleanDataV2 <- function( dna_samples                     ## DNA samples file  
                        ...
                        , age.label.lookup = NULL)#age.lookup.table)
   {
   ...
   temp<-temp1<-as.character(levels(myData$Age.orig))
   temp<-toupper(temp)#----upper case all
   temp<-gsub("\\s", "", temp)#---remove blank spaces
   data.frame(temp1,temp)
   #temp1<-paste("age: ",temp)# doing this because otherwise some values get read as dates in excel
   #----export a list of age-"categories" that will have to be transcribed
   
   #age.labels<-sort(unique(temp1[(is.na(as.numeric(temp)) | as.numeric(temp)>1000) & temp!="" ]))
   
   
   myData$Age.orig2<-myData$Age.orig
   sort(unique(myData$Age.orig2))
   levels(myData$Age.orig2)<-temp
   length(temp)
   length(levels(myData$Age.orig2))
   
   #---merge with info from lookup table
   
   sort(unique(myData$Age.orig2))
   sort(unique(age.label.lookup$age.label))
   
   myData<-merge(myData,age.label.lookup[,-1],by.x="Age.orig2",by.y="age.label",all.x=TRUE)
   table(myData$age)
   ...
   age<-floor(as.numeric(as.character(myData$Age.orig2[numeric.age.records])))
   myData[numeric.age.records,c("min.age","max.age","age")]<-age
   myData$min.age<-as.numeric(myData$min.age)
   myData$max.age<-as.numeric(myData$max.age)
