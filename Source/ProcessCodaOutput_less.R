...
ProcessCodaOutput_less <- function(x,DIC=F, params.omit=NULL, verbose=TRUE) {
   ...
   q2.5 <- q97.5 <- as.list(rep(NA,length(params.simple)))
   names(sims.list) <- names(means) <- names(rhat) <- names(n.eff) <- params.simple
   names(se) <- names(q2.5) <- names(q97.5) <- params.simple
   ...
   if(DIC){
      ...
      if(verbose){cat('\nDone.','\n')}
      return(list(sims.list=sims.list,mean=means,sd=se,lci=q2.5,uci=q97.5,
                  Rhat=rhat,n.eff=n.eff,pD=pd,DIC=dic,colnames.sims=colnames.sims))
   } else {
      #Otherwise return list without pD/DIC
      if(verbose){cat('\nDone.','\n')}
      return(list(sims.list=sims.list,mean=means,sd=se,lci=q2.5,uci=q97.5,
                  Rhat=rhat,n.eff=n.eff,colnames.sims=colnames.sims))
   }
