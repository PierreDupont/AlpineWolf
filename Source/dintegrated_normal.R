...
#'
#' The \code{dintegrated_normal} distribution is a NIMBLE custom distribution which can be used to model 
...
#' 
#' The \code{dintegrated_normal} distribution incorporates three features to increase computation efficiency (see Turek et al., 2021 <doi.org/10.1002/ecs2.3385>  for more details):
#' \enumerate{
...
#' }
#' 
#' The \code{dintegrated_normal} distribution requires x- and y- detector coordinates (\emph{trapCoords}) and activity centers coordinates (\emph{s}) to be scaled to the habitat grid (\emph{habitatGrid}) using the (\code{\link{scaleCoordsToHabitatGrid}} function.)
#'
...
#' dintegrated_normal(x=SparseY$y[i,,1],
...
#' dintegrated_normal(x=SparseY$yCombined[i,,1],
#'                    size=rep(1,4),
#'                    p0 = p0,
#'                    sigma= sigma, 
#'                    s=s[i,1:2],
...
#' @rdname dintegrated_normal
#' @export
dintegrated_normal <- nimbleFunction(
