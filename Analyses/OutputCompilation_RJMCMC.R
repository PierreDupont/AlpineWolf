...
# iter <- seq(1,dim(res_sxy$sims.list$z)[1],length.out = 10000)
# 
# 
# 
...
#   for(ss in 1:3){
#     thisStatus <- (res_sxy$sims.list$z[iter,augmented] == 1) &
#       (res_sxy$sims.list$sex[iter,augmented] == s) &
#       (res_sxy$sims.list$status[iter,augmented] == ss)
# 
...
#                         function(x){
#                           intens <- c(exp((res$sims.list$betaHab[x, ] * res$sims.list$zRJ[x, ]) %*% t(nimData$hab.covs)))
...
#                       ncol = ncol(habitat.r),
#                       byrow = TRUE)
# iter <- seq(1,dim(res_sxy$sims.list$z)[1],length.out = 5000)
# WA_plot <- GetDensity(
#   sx = s.rescaled[iter, ,1],
#   sy = s.rescaled[iter, ,2],
#   z = res_sxy$sims.list$z[iter, ],
...
col <- colFunc(100)
{
  pdf(file = file.path(thisDir, "figures", paste0(modelName, "_density_map.pdf" )),
      width = 10, height = 8)
...
{
pdf(file = file.path(thisDir, "figures", paste0(modelName, "_realizedVSpredicted_density.pdf" )),
    width = 16, height = 8)
par(mfrow = c(1,2), mar = c(4,4,0,0),bg="white")
plot(st_geometry(st_union(st_intersection(countries,studyArea))),
     col = "white", border = "black")
plot( st_geometry(countries), add = T, lwd = 1, border = "black", col = "gray80")
...
plot( meanDensity.R, breaks = cuts, col = col,
      ...
      axis.args = list( at = round(seq(0,maxDens,length.out = 5), digits = 5),
                        ...
                        cex.axis = 0.6),
      legend.args = list( text = 'Density (wolves/km2)', line = -2,
...
n.iterations <- nrow(res$sims.list$betaHab)
...
betas.wide$iteration <- zRJ.wide$iteration <- 1:n.iterations
...
zRJ.df <- melt(zRJ.wide, id.vars = c("iteration", "model"))
names(zRJ.df) <- c("iteration", "model", "variable", "value")
betas.df <-  melt(betas.wide, id.vars = c("iteration", "model"))
names(betas.df) <-  c("iteration", "model", "variable", "value")
...
betas.df <- betas.df[order(betas.df$variable, betas.df$model), ]
zRJ.df <- zRJ.df[order(zRJ.df$variable, zRJ.df$model), ]

myfun1 <- function(x) 1:length(x)

temp <- betas.df %>% group_by(variable, model) %>%
...
betas.df %>% group_by(variable) %>%
  summarise(mean = mean(value),
            qLow = quantile(value,0.025),
            qUp = quantile(value,0.975))


##---- MODEL WEIGHTS
pdf(file = file.path(thisDir, "figures", paste0(modelName, "_ModelWeights.pdf" )),
    width = 8, height = 8)
...
ggplot(data = aggr,
  ...
  theme(axis.text.x = element_text( angle = 45,
                                    vjust = 1,
                                    hjust = 1),
        plot.margin = margin(t = 0, r = 0, b = 3, l = 4, unit = "cm"),
...
pdf(file = file.path(thisDir, "figures", paste0(modelName, "_CoeffInclusionAll.pdf" )),
    width = 8, height = 8)
...
pdf(file = file.path(thisDir, "figures", paste0(modelName, "_CoeffInclusionModel.pdf" )),
    width = 8, height = 8)
...
par(mfrow = c(1,1), mar = c(6,6,0,0))
covNames <- names(nimData$hab.covs)
labNames <- 
cols <- met.brewer(name = "Isfahan1", n = length(covNames), type = "discrete")
pred.hab.covs <- apply(nimData$hab.covs, 2, function(c){seq(min(c),max(c),length.out = 100)})
mean.hab.covs <- apply(nimData$hab.covs, 2, mean)

for(b in 1:ncol(res$sims.list$betaHab)){
  pdf(file = file.path(thisDir, "figures", paste0(modelName, "_", covNames[b], ".pdf" )),
      width = 7, height = 7)  
  ##-- Calculate intensity
  ...
  axis(2, at = seq(0,floor(maxD),length.out = 6),
       labels = seq(0,floor(maxD),length.out = 6),
...
##---- Create empty table
popCompo  <-  matrix(NA, nrow = 3, ncol = 3)
colnames(popCompo) <- status
rownames(popCompo) <- c(sex, "Total")
...
percentUndetected[[3]][[4]] <- (WA_undetected[[1]][[1]]$PosteriorAllRegions + 
                                 ...
                                 WA_undetected[[2]][[3]]$PosteriorAllRegions) / (WA_status[[1]][[1]]$PosteriorAllRegions +
                                                                                    ...
                                                                                    WA_status[[2]][[1]]$PosteriorAllRegions +                                                                                  WA_status[[2]][[3]]$PosteriorAllRegions)
