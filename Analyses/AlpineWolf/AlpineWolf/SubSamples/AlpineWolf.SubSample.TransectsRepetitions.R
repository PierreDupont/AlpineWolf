...
## -----------------------------------------------------------------------------
## ------ III. SUBSAMPLE TRANSECT REPETITIONS ------
##---- Buffer all transects with 500m radius
transects_b <- st_buffer(transects, dist = 500)

##---- Split NGS data into systematic and opportunistic samples
ngs_sys <- st_filter(ngs,transects_b)
ngs_opp <- ngs[!ngs$Sample.ID %in% ngs_sys$Sample.ID, ]

# ##---- Assign each systematic smaple to a transect
# ngs_sys$transect_ID <- unlist(lapply(st_intersects(ngs_sys, transects_b), function(x)x[1]))
...
for (num in sim_names){
  for (rep in 1:max.rep) {
    ...
    trans3_b <- filter(transects_b, path_id %in% trans3$path_id)
    ...
    for(c in 1:4){
      ...
      save( modelCode,
            ...
            file = file.path(thisDir, "input",
                             paste0(modelName, "_", num, "_", rep, "_", c, ".RData")))
    }#c
