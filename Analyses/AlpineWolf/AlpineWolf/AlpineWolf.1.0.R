...
##-- PLot density +individual centroids
plot( habitat$polygon, border = grey(0.3))
plot( meanDensity.R, add = T,
      breaks = cuts, col = col,
      axes = F, box = F, bty = "n", legend = F)
...
plot(centroids,
     ...
     bg = adjustcolor("red",0.2),
     col = "red")
mtext( text = paste0("Wolf density with\ndetected individuals' centroid\n(N = ",
                     nrow(centroids), ")"),
       side = 3, font = 2, cex = 2)
...
for(b in 1:ncol(res$sims.list$betaHab)){
  c(exp(res$mean$betaHab %*% t(nimData$hab.covs)))
}

p0.posterior1 <- do.call( rbind,
                          lapply(1:length(res$sims.list$beta),
                                 function(x){
...
## ------     3.1. DETECTION MAP ------
detectors$grid$p0 <- c(ilogit(logit(res$mean$p0[1,1]) +
                                res$mean$betaDet %*% t(nimData$det.covs)))
plot(detectors$grid[,"p0"])


## ------     3.2. DETECTION EFFECT PLOT ------
# par(mar = c(20,10,15,5))
# detCovs <- seq(min(nimData$det.covs), max(nimData$det.covs), length.out = 100)
# p0.posterior1 <- do.call( rbind,
#                           lapply(1:length(res$sims.list$betaTrap),
#                                  function(x){
#                                    ilogit(logit(res$sims.list$p0[x,1]) +
...
# 
## ------   4. PARAMETER TABLES ------
params.simple <- names(res$mean)[c(1:6,8,9)]
