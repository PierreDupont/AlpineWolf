...
## MODEL NAME 
modelName = "AlpineWolf.5.2_noopp"
...
##---- Filter systematic and opportunistic data
...
modelCode <- nimbleCode({
  ...
  for(sss in 1:2){
    theta[1:n.states,sss] ~ ddirch(alpha[1:n.states,sss])
  }#ss
  ...
  for(ss in 1:n.states){
    for(sss in 1:2){
      p0[ss,sss] ~ dunif(0,0.5)
      sigma[ss,sss] ~ dunif(0,12)
      logit(p0Traps[ss,sss,1:n.detectors]) <- logit(p0[ss,sss]) + 
