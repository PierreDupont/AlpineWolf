...
nimData <- list( y = yCombined.aug,
                 ...
                 hab.covs = c(scale(st_drop_geometry(habitat$grid[ ,c("forest")]))),
...
intensity.r[intensity.r[] > 0] <- exp(res$mean$betaHab*nimData$hab.covs)
...
plot( relativeInt.r, breaks = cuts, col = col,
      ...
      axis.args = list( at = round(seq(0,maxDens,length.out = 5), digits = 5),
                        ...
                        cex.axis = 0.6),
      legend.args = list( text = 'Relative density (% of pop.25km2)', line = -2,
