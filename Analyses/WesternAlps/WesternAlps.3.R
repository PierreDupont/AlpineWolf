...
studyArea_west <- st_sfc(st_cast(st_difference(st_buffer(studyArea,2000), cutline),"POLYGON")[[1]])
...
##---- Create a detector grid from buffered transects with the desired resolution
...
status[status == "pup"] <- 2
status[status == "pup 2016"] <- 2
status[status == "pup 2017"] <- 2
...
status <- MakeAugmentation( y = status,
                            aug.factor = data$aug.factor,
                            replace.value = NA)
...
##-- PROCESSED DATA
plot(studyArea_west, col = adjustcolor("gray60", alpha.f = 0.5), border = F)
...
numDetsPerDet <- apply(detMat, 2, sum)
