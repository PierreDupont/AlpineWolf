...
habitat$grida <- habitat$grid[c(4,5,6,13,17)] 

# 13,17
# " Log Human Population Density", "Wolf Historical Presence",
names(habitat$grida) <- c("Forest Cover", "Low Natural Vegetation Cover", "Bare Rock Cover","Log Human Population Density", "Wolf Historical Presence", "geometry")
plot(habitat$grida, 
     breaks = "equal")


plot(habitat$grida["Forest Cover"], breaks = "equal")
# mtext( text = "Wolf Historical Presence", side = 3, font = 2)


## ------ 01.PLOT DETECTORS COV  ------

detectors$grida <- detectors$grid[c(3,5,7,17,19)] 
names(detectors$grida) <- c("Transects Length", "Transects operators experience", "Snowfall","Log Human Population Density" ,"geometry")

detectors$grida$`Transects Length` <- log(detectors$grida$`Transects Length`)
detectors$grida$`Transects Length` <- rescale(detectors$grida$`Transects Length`, to = c(0, 10))
detectors$grida$`Transects Length`[is.na(detectors$grida$`Transects Length`)] <- 0


# 13,17
# " Log Human Population Density", "Wolf Historical Presence",
plot(detectors$grida, 
     breaks = "equal")

plot(detectors$grida["Transects operators experience"], breaks = "equal")
# mtext( text = "Wolf Historical Presence", side = 3, font = 2)
...
iter <- seq(1,dim(res$sims.list$z)[1],length.out = 1000)
...
alpDensity.R[] <- WA_regions$MeanCell
...
##---- Create empty table
abundanceTable  <-  matrix(NA, nrow = 3, ncol = 12)
colnames(abundanceTable) <- c(rep("Alpha",3), rep("Pups",3), rep("Others",3), rep("Total",3))
rownames(abundanceTable) <- c("Region", row.names(WA_alps$summary))
...
for(ss in 1:length(status)){
  ...
  TotalTmp <- WA_status[[sex[1]]][[status[ss]]]$PosteriorAllRegions + WA_status[[sex[2]]][[status[ss]]]$PosteriorAllRegions
...
abundanceTable[2:nrow(abundanceTable),colPos[3]] <- paste0(
  format(round(WA_alps$summary[,"mean"],1), nsmall = 1), " (",
  WA_alps$summary[,"95%CILow"], "-",
  WA_alps$summary[,"95%CIHigh"], ")")
