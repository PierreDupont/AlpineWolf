...
#   pdf(file = file.path(thisDir, paste0(modelName, "_density_legend3.pdf" )),
#       width = 1.75, height = 5)
#   par(mar=c(1,1,1,4),las=1,cex.axis=1.6,xaxs="i",yaxs="i")#,bg="black")
#   plot(1,type="n",axes=FALSE,ylim=c(1,100),xlim=c(0,1),xlab="",ylab="")
#   resolution<-prod(res(meanDensity.R)/1000)
...
pdf(file = file.path(thisDir, "figures", paste0(modelName, "_RJMCMC.pdf" )),
    width = 8, height = 8)
...
tmp <- betas.df %>%
  ...
  summarize( mean = mean(value),
             ...
             upCI = quantile(value, 0.975), 
             weight = unique(weight))

tmp[tmp$model == "bare rock+herbaceous+pop+IUCN", ]

tmp[tmp$model == "herbaceous+forest+pop+IUCN", ]




## ------     2.1.3. DENSITY EFFECT PLOTS ------
...
labNames <- c("Transect length", "Transect operator's experience", "Average snowfall", "Zone", "log(human population density)")
cols <- met.brewer(name="Isfahan1",n=length(covNames),type="discrete")
...
for(b in 1:ncol(res$sims.list$betaDet)){
  pdf(file = file.path(thisDir, "figures", paste0(modelName, "_", covNames[b], ".pdf" )),
      width = 7, height = 7)  
  par(mfrow = c(1,1), mar = c(6,6,0,0))# bg = "black",col.lab = "white", col.axis="white")
  ...
  plot( x = pred.det.covs[ ,b],
        ...
        ylab = expression(p[0]), xlab = labNames[b], axes = FALSE, cex.lab = 2)
...
propAlpha <- (WA_status[[1]]$Alpha$PosteriorAllRegions + WA_status[[2]]$Alpha$PosteriorAllRegions)/WA_regions$PosteriorAllRegions
mean(propAlpha)
quantile(propAlpha,c(0.025,0.975))

propPup <- (WA_status[[1]]$Pups$PosteriorAllRegions + WA_status[[2]]$Pups$PosteriorAllRegions)/WA_regions$PosteriorAllRegions
mean(propPup)
quantile(propPup,c(0.025,0.975))

propOther <- (WA_status[[1]]$Others$PosteriorAllRegions + WA_status[[2]]$Others$PosteriorAllRegions+WA_status[[1]]$Alpha$PosteriorAllRegions + WA_status[[2]]$Alpha$PosteriorAllRegions)/WA_regions$PosteriorAllRegions
mean(propOther)
quantile(propOther,c(0.025,0.975))



##----- Proportion of population undetected
propUndet <- 1-449/WA_regions$PosteriorAllRegions
mean(propUndet)
quantile(propUndet, 0.025)
quantile(propUndet, 0.975)
