...
grid <- st_as_sf(st_make_grid( studyArea_west,
  ...
  st_intersection(studyArea_west) 
grid$id <- 1:nrow(grid)
plot(grid)
...
nimData <- list( y = yCombined.aug,
                 ...
                 hab.covs = cbind(scale(st_drop_geometry(habitat$grid[ ,c("elev")])),
                                  scale(st_drop_geometry(habitat$grid[ ,c("elev")]))^2),
                 det.covs = st_drop_geometry(detectors$grid[ ,c("snow_fall","transect_L")]),
...
row.names(hab.rgmx) <- c("habitat","comp")
...
meanDensity.R <- habitat.r
meanDensity.R[ ] <- RD_Density$MeanCell
meanDensity.R[is.na(habitat$comparison[])] <- NA
...
mtext( text = paste( "N = ", round(RD_Density$summary["comp",1],1),
                     " [", round(RD_Density$summary["comp",4],1), " ; ",
                     round(RD_Density$summary["comp",5],1), "]", sep = ""),
       side = 1, font = 2, cex = 1.5)
...
intensity.r[intensity.r[] > 0] <- exp(
  ...
  res$mean$betaBNF * nimData$REGION_BNF +
    res$mean$betaBNFp * nimData$REGION_BNFp +
    res$mean$betaNEU * nimData$REGION_NEU +
    res$mean$betaSUMp * nimData$REGION_SUMp +
    res$mean$betaR * nimData$R +
    res$mean$betaDEM * nimData$DEM +
    res$mean$betaDEMR * nimData$DEM * nimData$R)
...
## -----------------------------------------------------------------------------
## ------ VI. EXPORT RESULTS REPORT AS .pdf ------
{
