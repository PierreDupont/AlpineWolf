...
##-- This script presents a simulation study based on the Alpine wolf set-up to 
##-- investigate the possibility to identify and estimate spatial variation in 
##-- wolf home-range size using SCR model.
##-- Specifically, we use the Alpine wolf habitat definition, spatial covariates
##-- and SCR parameter estimates from Marucco et al. (2023) to define the simulation
##-- state space.
##-- In addition, we simulate spatial variation in sigma based on local wolf 
##-- population density. 
...
##---- MODEL NAME 
modelName = "simulation.0.1"
thisDir <- file.path(analysisDir, "SpatialSigma", modelName)
...
count <- read_sf(file.path(dataDir, "GISData/Countries/Countries_WGS84.shp"))
temp <- count[count$CNTRY_NAME %in% c("Albania", "Austria",
                                      "Bosnia and Herzegovina",
                                      "Bulgaria", "France",
                                      "Germany", "Greece", "Croatia",
...
plot(st_geometry(st_intersection(studyArea,countries)),
     add = T, col = "red", border = F)
...
iucn_2018$SPOIS <- ifelse(iucn_2018$SPOIS == "Sporadic", 1, 3)## Code back to numeric
...
## ------       1.2.8. DETECTOR COVARIATES DATAFRAME -----
det.covs = cbind.data.frame( "transect_L" = detectors$grid$`transect_L`,
...
## ------       2.2.8. HABITAT COVARIATES DATAFRAME ------
hab.covs = cbind.data.frame( "bare rock" = habitat$grid$`bare rock`,
...
## -----------------------------------------------------------------------------
## ------ III. SCR DATA SIMULATION ------
##---- Load SCR results
load("C:/Users/pidu/AQEG Dropbox/AQEG Team Folder/AlpineWolf/02_Analysis/AlpineWolf.5.2_RJMCMC/AlpineWolf.5.2_RJMCMC_mcmc.RData")

##---- Sample one iteration from the posterior MCMC 
iter <- sample(length(res$sims.list$N), 1)




## ------   1. SIMULATE s ----
##---- Calculate point process intensity based on selected SCR posterior estimates
habIntensity <- exp(as.matrix(hab.covs) %*% res$sims.list$betaHab[iter, ])
sumHabIntensity <- sum(habIntensity)
logHabIntensity<- log(habIntensity)
logSumHabIntensity <- log(sumHabIntensity)

##---- Simulate individual AC locations
s.full <- do.call(rbind, lapply(1:sim$N, function(i){
  rbernppAC( n = 1,
             lowerCoords = habitat$lowerCoords,
             upperCoords = habitat$upperCoords,
             logIntensities = logHabIntensity,
             logSumIntensity = logSumHabIntensity,
             habitatGrid = habitat$matrix,
             numGridRows = nrow(habitat$matrix),
             numGridCols = ncol(habitat$matrix))
}))

##-- Scale to habitat grid
scaled.s <- scaleCoordsToHabitatGrid(
  coordsData = s.full,
  coordsHabitatGridCenter = habitat$coords)$coordsDataScaled 

##-- Calculate realized density
ACdensity <- getDensity(
  s = scaled.s,
  habitatGrid = localObjects$habitatGrid,
  indicator = rep(1,sim$N),
  numWindows = habitat$n.HabWindows,
  n.individuals = sim$N)
densCov <- log(ACdensity+1) - 0.2



## ------   2. SIMULATE sex, status -----
##---- Sex
rho <- res$sims.list$rho[iter]
sex.full <- rbinom( n = sim$N,
                    size = 1,
                    prob = rho)

##---- Status
theta <- res$sims.list$theta[iter, , ] 
status.full <- sapply(1:sim$N,
                      function(i)rcat( n = 1, prob = theta[ ,sex.full[i]+1]))



## ------   3. SIMULATE p0, sigma, y -----
betaDet <- res$sims.list$betaDet[iter, ]
p0 <- res$sims.list$p0[iter, , ]
sigma0 <- res$sims.list$sigma[iter, , ]

y.full <- matrix(NA, sim$N, sim$numMaxDetections)
for(i in 1:sim$N){
  p0Traps <- ilogit(logit(p0[status.full[i],sex.full[i]+1]) + as.matrix(det.covs) %*% betaDet)
  
  sID <- localObjects$habitatGrid[trunc(scaled.s[i,2]) + 1, trunc(scaled.s[i,1]) + 1]
  
  sigma <- exp(log(sigma0[status.full[i],sex.full[i]+1]) +
                 sim$betaDens * densCov[sID] +
                 inprod(sim$betaSigma,  hab.covs[sID, ]))
  
  y.full[i, ] <- rbinomLocal_normal( n = 1,
                                     size = detectors$size,
                                     p0Traps = p0Traps,
                                     sigma = sigma,
                                     s = scaled.s[i, ],
                                     ...
                                     habitatGrid = localObjects$habitatGrid,
                                     indicator = 1,
                                     lengthYCombined = sim$numMaxDetections)
}#i
...
for(ss in 1:2){
  for(s in 1:3){
    sigma <- c(exp(log(sigma0[s,ss]) +
                     sim$betaDens * densCov +
                     inprod(sim$betaSigma, hab.covs)))*10
    
    habitat$grid[ ,paste0("sigma_",sex[ss],"_",status[s])] <- sigma
    
    habitat$grid[ ,paste0("HR_area_",sex[ss],"_",status[s])] <- pi*(sigma*sqrt(qchisq(0.95,2)))^2
  }#s
}#ss
plot(habitat$grid[ ,c("HR_area_female_alpha","HR_area_male_alpha",
                      "HR_area_female_pup","HR_area_male_pup",
                      "HR_area_female_other","HR_area_male_other")],
     key.pos = 4,
...
## ------   4. FORMAT SIMULATED DATA ------
n.detected <- sum(y.full[ ,1]>0)

##---- Reduce detections history to detected individuals
y.sim <- y.full[y.full[ ,1]>0, ]
sex.sim <- sex.full[y.full[ ,1]>0]
status.sim <- status.full[y.full[ ,1]>0]

##---- Data augmentation
yCombined.aug <- MakeAugmentation( y = y.sim,
                                   aug.factor = data$aug.factor,
                                   replace.value = 0)

sex.aug <- MakeAugmentation( y = sex.sim,
                             aug.factor = data$aug.factor,
                             replace.value = NA)

status.aug <- MakeAugmentation( y = status.sim,
                                aug.factor = data$aug.factor,
                                replace.value = NA)
...
nimConstants <- list( n.individuals = nrow(yCombined.aug),
                      n.maxDets = ncol(yCombined.aug),
                      n.habWindows = habitat$n.HabWindows,
                      n.detectors = detectors$n.detectors, 
                      n.habCovs = ncol(hab.covs),
                      n.detCovs = ncol(det.covs),
                      n.states = 3,
...
## ------   3. SAVE THE INPUT ------
numDetMax <- trunc((sim$numMaxDetections-1)/2)
detNums <- yCombined.aug[ ,1]
detIndices <- yCombined.aug[ ,(numDetMax+2):dim(yCombined.aug)[2]]
...
for(c in 1:8){
  ...
  for(i in (n.detected + 1):nimConstants$n.individuals){
    s.init[i, ] <- rbernppAC( n = 1,
                              lowerCoords = nimInput$data$lowerHabCoords,
                              upperCoords = nimInput$data$upperHabCoords,
                              logIntensities = log(rep(1,habitat$n.HabWindows)),
                              logSumIntensity = log(sum(rep(1,habitat$n.HabWindows))),
                              habitatGrid = nimInput$data$habitatGrid,
                              numGridRows = nrow(nimInput$data$habitatGrid),
                              numGridCols = ncol(nimInput$data$habitatGrid))
  }#i
  
  sex.init <- rbinom(n = nimConstants$n.individuals, 1, prob = 0.5)
  sex.init[!is.na(nimInput$data$sex)] <- NA
  sex1.init <- sex.init + 1
  
  status.init <- rcat(n = nimConstants$n.individuals, prob = c(0.3,0.6,0.1))
  status.init[!is.na(nimInput$data$status)] <- NA
  
  z.init <- rbinom(n = nimConstants$n.individuals, 1, prob = 0.1)
  z.init[!is.na(nimInput$data$z)] <- NA
...
##---- Traceplots
pdf(file = file.path(thisDir, "figures", paste0(modelName, "_traceplots.pdf")))
plot(nimOutput)
graphics.off()
...
# save(res, res2, file = file.path(thisDir, paste0(modelName,"_mcmc.RData")))



## ------   2. CALCULATE AC-BASED DENSITY ------
...
row.names(regions.rgmx) <- c("Piemonte","Valle d'Aosta","Liguria")   
...
for(s in 0:1){
  WA_status[[s+1]] <- list()
  for(ss in 1:3){
    status <- (res2$sims.list$z == 1) &
      (res2$sims.list$sex == s) &
      (res2$sims.list$status == ss)
...
intensity.r[intensity.r[] > 0] <- c(exp(res$mean$betaHabDens %*% t(nimInput$data$hab.covs)))
...
## ------     3.2. DENSITY EFFECT PLOT ------
par(mfrow = c(2,3), mar = c(6,6,0,0))
covNames <- names(nimInput$data$hab.covs)# c("alpine", "forest", "IUCN")
pred.hab.covs <- apply(nimInput$data$hab.covs,
                       2,
                       function(c){
                         seq(min(c),
                             max(c),
                             length.out = 100)})
mean.hab.covs <- apply(nimInput$data$hab.covs, 2, mean)
cols <- hcl.colors(length(mean.hab.covs))

for(b in 1:ncol(res$sims.list$betaHabDens)){
  intensity <- do.call( rbind,
                        lapply(1:length(res$sims.list$betaHabDens[ ,b]),
                               function(x){
                                 exp(res$sims.list$betaHabDens[x,b] * pred.hab.covs[ ,b] + 
                                       sum(res$sims.list$betaHabDens[x,-b] * mean.hab.covs[-b]))
                               }))
...
for(ss in 1:2){
  for(s in 1:3){
    sigma <- c(exp(log(res$mean$sigma0[s,ss]) +
                     res$mean$betaHabDet %*% t(nimInput$data$hab.covs)))*10
    habitat$grid[ ,paste0("sigma_",sex[ss],"_",status[s])] <- sigma
...
## ALPHA
png( file = paste0("C:/Users/pidu/OneDrive - Norwegian University of Life Sciences/PROJECTS/ROVQUANT/TALKS/Wolves Across Borders_May_2023/HR_",
                   "HR_area_female_alpha.png"),
     width = 400,height = 400, bg = "transparent")
plot(habitat$grid[ ,"HR_area_female_alpha"],
     main = "", 
     key.pos = 4,
     breaks = seq(0,350,25))
dev.off()
...
png( file = paste0("C:/Users/pidu/OneDrive - Norwegian University of Life Sciences/PROJECTS/ROVQUANT/TALKS/Wolves Across Borders_May_2023/HR_",
                   "HR_area_male_alpha.png"),
     width = 400,height = 400, bg = "transparent")
plot(habitat$grid[ ,"HR_area_male_alpha"],
     main = "", 
     key.pos = 4,
     breaks = seq(0,350,25))
dev.off()
...
## PUPS
png( file = paste0("C:/Users/pidu/OneDrive - Norwegian University of Life Sciences/PROJECTS/ROVQUANT/TALKS/Wolves Across Borders_May_2023/HR_",
                   "HR_area_female_pup.png"),
     width = 400,height = 400, bg = "transparent")
plot(habitat$grid[ ,"HR_area_female_pup"],
     main = "", 
     key.pos = 4,
     breaks = seq(0,350,25))
dev.off()
...
png( file = paste0("C:/Users/pidu/OneDrive - Norwegian University of Life Sciences/PROJECTS/ROVQUANT/TALKS/Wolves Across Borders_May_2023/HR_",
                   "HR_area_male_pup.png"),
     width = 400,height = 400, bg = "transparent")
plot(habitat$grid[ ,"HR_area_male_pup"],
     main = "", 
     key.pos = 4,
     breaks = seq(0,350,25))
dev.off()
...
## Others
plot(habitat$grid[ ,"HR_area_female_other"],
     main = "", 
     key.pos = 4,
     breaks = seq(0,4000,500))

plot(habitat$grid[ ,"HR_area_male_other"],
     main = "", 
     key.pos = 4,
     breaks = seq(0,4000,500))
...
covNames <- names(nimInput$data$hab.covs)
pred.hab.covs <- apply(nimInput$data$hab.covs,
                       2,
                       function(c){
                         seq(min(c),
...
for(b in 1:ncol(res$sims.list$betaHabDet)){
  ...
  area<-pi*(sigma*sqrt(qchisq(0.95,2)))^2
  mean.int <- colMeans(area)
  quant.int <- apply(area, 2, function(x)quantile(x,c(0.025,0.5,0.975)))
  maxD <- round(max(quant.int),3)
  png( file = paste0("C:/Users/pidu/OneDrive - Norwegian University of Life Sciences/PROJECTS/ROVQUANT/TALKS/Wolves Across Borders_May_2023/HR_",
                     covNames[b],".png"),
       width = 400,height = 400, bg = "transparent")
  ...
  axis(1,
       ...
       labels = xLabels, cex = 2,
       tck = 0.01, las = 1, hadj = 0.5)
  axis(2, at = ceiling(seq(0,maxD,length.out = 6)),
       labels = ceiling(seq(0,maxD,length.out = 6)),
       tck = 0.01, las = 1, cex = 2)
...
for(ss in 1:2){
  for(s in 1:3){
    detectors$grid[ ,paste0("p0_",sex[ss],"_",status[s])] <- 
      c(ilogit(logit(res$mean$p0[s,ss]) +
                 res$mean$betaDet %*% t(nimInput$data$det.covs)))
  }#s
}#ss
plot(detectors$grid[ ,c("p0_female_alpha","p0_female_pup","p0_female_other", 
                        "p0_male_alpha","p0_male_pup","p0_male_other")],
     key.pos = 4, breaks = exp(seq(-8,0,1))) 




## ------     5.2. p0 EFFECT PLOT ------
par(mfrow = c(2,3), mar = c(8,8,0,4))
covNames <- names(nimInput$data$det.covs)
pred.det.covs <- apply(nimInput$data$det.covs,
                       2,
                       function(c){
                         seq(min(c),
                             max(c),
                             length.out = 100)})
mean.det.covs <- apply(nimInput$data$det.covs, 2, mean)
