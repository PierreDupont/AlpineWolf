...
#' @param prob Vector of success probabilities on each trial. Here, these correspond to the size of each group. 
...
dtbinom <- nimbleFunction(
  run = function( x = double(0),                   ## Number of wolves detected together 
    ...
    normCst <- 0
    for(i in a:b){
      normCst <- normCst + sum(dbinom( x = i,
                                       size = size,
                                       prob = prob,
                                       log = 0))
    }#i
