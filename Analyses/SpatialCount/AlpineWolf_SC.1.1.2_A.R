...
sigma <- cbind.data.frame(
  mean=c(3.3,3.5,2.4,4.08,3.33,3.49),
  sd = c(0.3,0.25,0.14,0.2,0.25,0.2))
...
plot(density(x = rgamma(10000,shape = 13,scale = 0.26)),col="red",lwd=3)
...
for(r in 1:length(gammas$scale)){
  points((density(x = rgamma(10000,
                             shape = gammas$shape[r],
                             scale = gammas$scale[r]))),
         col = r, type = "l")
}
