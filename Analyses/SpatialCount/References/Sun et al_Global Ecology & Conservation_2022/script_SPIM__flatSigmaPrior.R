...
#data and prep
study<-"Algar"
source("SPIMfunction_withFlatSigmaPrior.R")

species1<-"caribou"
timeFrame<-"3mos"
dataType<-"grpSize1"#grpSize1" #all
year<- 2018
chain<-2
...
for(i in 1:nrow(y.obs_long)){
  ...
  #whichOcc<-which(colnames(camOp_focal)%in%as.character(as.Date(y.obs_long$Date_Time.Captured[i], "%m/%d/%y")-366))
...
G.obs<-cbind(y.obs_long$Sex,y.obs_long$Antler.pts,y.obs_long$Collar) # i . # cat vars
head(G.obs)
...
#MCMC stuff
niter=250000 #how long to run the MCMC chain. 
...
a=Sys.time()
out<-mcmc.CatSPIM_flat(dataSPIM,tf=camOp_focal,
...
b=Sys.time()
b-a
save.image(file=paste0("/uninformativePriors/outputSPIM_",LowStr(commonName),"_",study,"_",year,"_",timeFrame,"_",dataType,"_chain",chain,".RData"))
