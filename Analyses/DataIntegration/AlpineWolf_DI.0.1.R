...
## ------   4. PICTURES/SIGHTENING DATA ------
## ------   4.1 CAMERA TRAPS INFO ------
...
## ------   5. SNOW-TRAKCS DATA ------
##---- Load starting points for snowtracks 
st <- read_sf(file.path(dataDir,"GISData/Snowtracks/ST_2021.shp"))
#-- Filter out C3 data
st_t <- filter(st,Scalp %in% 'C2')  
dim(st_t)

##---- Use lubridate to clean dates
st_t$date <- parse_date_time(st_t$Data, orders = c('dmy','mdy','ymd'))
st_t$Month <- as.numeric(format(st_t$date,"%m"))
st_t$n_wolves <- as.numeric(st_t$n_lupi)

##---- Filter out samples outside the monitoring period
st_tt <- filter(st_t,Month %in% c(1,2,3,4,10,11,12)) 



##--- PLOT ALL DATA ----
plot(st_geometry(studyArea))
plot(st_geometry(pics), col = "blue", add = T)
plot(st_geometry(st_tt),col = "red",add=T )
# plot(st_geometry(transects), col = "red", add = T)
plot(st_geometry(ngs), pch = 3, add = T)
