...
nimParams <- c("N", "p0", "betaDet", "sigma", "psi", "z", "s")
...
save(WA_Density, WA_Comp,
file = file.path(thisDir, paste0(modelName,"_density.RData")))

## LOAD AC-based DENSITY FILES
load(file.path(thisDir, paste0(modelName, "_density.RData")))
...
detectors$main.detector.sp$p0 <- ilogit(logit(res$mean$p0[nimData$trapIntIndex]) +
...
## ------   4. PARAMETER TABLES ------
params.simple <- names(res$mean)[c(1:4,6)]
params.means <- unlist(lapply(params.simple, function(x)res$mean[[x]][1]))
params.sd <- unlist(lapply(params.simple, function(x)res$sd[[x]][1]))
params.q2.5 <- unlist(lapply(params.simple, function(x)res$q2.5[[x]][1]))
params.q97.5 <- unlist(lapply(params.simple, function(x)res$q97.5[[x]][1]))
params.Rhat <- unlist(lapply(params.simple, function(x)res$Rhat[[x]][1]))
params.n.eff <- unlist(lapply(params.simple, function(x)res$n.eff[[x]][1]))
...
{## ------ IV. EXPORT RESULTS REPORT AS .pdf -----
  ...
  params.means <- unlist(lapply(params.simple, function(x)myResults_M$mean[[x]][1]))
  params.sd <- unlist(lapply(params.simple, function(x)myResults_M$sd[[x]][1]))
  params.q2.5 <- unlist(lapply(params.simple, function(x)myResults_M$q2.5[[x]][1]))
  params.q97.5 <- unlist(lapply(params.simple, function(x)myResults_M$q97.5[[x]][1]))
  # params.Rhat <- unlist(lapply(params.simple, function(x)myResults_M$Rhat[[x]][1]))
  # params.n.eff <- unlist(lapply(params.simple, function(x)myResults_M$n.eff[[x]][1]))
